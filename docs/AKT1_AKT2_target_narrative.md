...
4. **C1_sd bottleneck in GI epithelial tumors — biology or assay?** CRC, stomach, lung adeno, and breast all show near-universal C0 pass (92–100%) but low C1_sd pass (19–24%) under pan-cancer Z-scores. Two competing explanations: (a) these tumors are genuinely committed to a uniform mesenchymal state — either fully epithelial or post-EMT mesenchymal — and lack the intermediate/plastic state the C1_sd criterion selects for; or (b) the C1_sd signal is suppressed by stromal dilution, where a dominant stromal fraction uniformly elevates all C1 genes, reducing within-sample variance regardless of tumor biology. Distinguishing these requires deconvolved RNA-seq or single-cell data from CRC vs. HCC (which has 95% C1_sd pass). If (a), these indications are genuinely less eligible and the indication ranking is correct. If (b), the C1_sd threshold needs recalibration against tumor-purity-matched samples and CRC eligibility would be substantially higher than 19.5%.
